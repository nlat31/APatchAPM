...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        new TabLayoutMediator(tabs, pager, (tab, pos) -> {
            ...
            else if (pos == 1) tab.setText(getString(R.string.tab_installer_spoof));
            else tab.setText(getString(R.string.tab_hide_dev));
        }).attach();

        Runnable updateClearVisibility = () -> btnClear.setVisibility(
            pager.getCurrentItem() == 0 ? android.view.View.VISIBLE : android.view.View.GONE
        );
        ...
        btnApply.setOnClickListener(v -> {
            ...
            ConfigStore.Config cfg = new ConfigStore.Config(
                ...
                vm.installerSpoofEnabled,
                vm.installerPackage,
                vm.hideDevOptionsEnabled,
                vm.hideDeveloperMode,
                vm.hideUsbDebug,
                vm.hideWirelessDebug,
                vm.hideDebugProperties,
                vm.hideDebugPropertiesInNative
            );
            RootShell.Result r = ConfigStore.writeConfig(cfg);
            if (r.code == 0) {
                Toast.makeText(this, "Saved to /data/adb/modules/envcloak/config.json", Toast.LENGTH_LONG).show();
            } else {
