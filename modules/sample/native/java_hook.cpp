...
#define ZMOD_HOOKER_CLASS "sample.Hooker"
#endif

#define LOG_TAG    "Sample/JavaHook"
...
void install_hooks(JNIEnv *env, const std::vector<uint8_t> &dex_data) {
    if (dex_data.empty()) {
        LOGW("[%s] classes.dex is empty; skip Java hooks", ZMOD_ID);
        return;
    }

    jclass loader_cls = find_class(env, "dalvik/system/InMemoryDexClassLoader");
    if (!loader_cls) {
        LOGE("[%s] InMemoryDexClassLoader not found", ZMOD_ID);
        return;
    }
    ...
    if (!ctor || !load_class) {
        env->ExceptionClear();
        LOGE("[%s] InMemoryDexClassLoader methods not found", ZMOD_ID);
        return;
    }
    ...
    if (!buf) {
        env->ExceptionClear();
        LOGE("[%s] NewDirectByteBuffer failed", ZMOD_ID);
        return;
    }
    ...
    if (!loader || env->ExceptionCheck()) {
        env->ExceptionClear();
        LOGE("[%s] Failed to create InMemoryDexClassLoader", ZMOD_ID);
        return;
    }
    ...
    if (env->ExceptionCheck() || !hooker_cls) {
        env->ExceptionClear();
        LOGE("[%s] Failed to load Hooker class: %s", ZMOD_ID, ZMOD_HOOKER_CLASS);
        return;
    }
    ...
    if (!hooker_ctor) {
        env->ExceptionClear();
        LOGE("[%s] Hooker.<init>() not found", ZMOD_ID);
        return;
    }
    jobject inst = env->NewObject(g_hooker_cls, hooker_ctor);
    if (!inst || env->ExceptionCheck()) {
        env->ExceptionClear();
        LOGE("[%s] Failed to instantiate Hooker", ZMOD_ID);
        return;
    }
